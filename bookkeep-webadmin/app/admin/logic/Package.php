...
class Package extends AdminBase
{
    ...
    public function createPack($data = [])
    {
        ...
        //1、一些数据文件处理
        $data_file_list = str2arr($data['data_file_list'], "\r\n");
        ...
        $pack_dir = $this->app_pack_path . $module_name . '/' . $module_name . DS;
        $file = new \lqf\File();
        ...
        //4，扩展文件复制文件
        if (!empty($data_file_list)) {
            $pack_dir_data = $pack_dir . 'data' . DS;
            foreach ($data_file_list as $key => $filepath) {
                if(empty($filepath)) continue;
                $source = ROOT_PATH . $filepath;//源位置
                $topath = $pack_dir_data . $filepath;//目的位置
                $datalog .= '<br>' . $source . '=>' . $topath;
                if (!file_exists($source)) {
                    throw_response_error('打包模数据文件不存在');
                }
                if (!is_dir($source)) {
        ...
        //5、压缩包zip文件
        $pack_zip = $this->app_pack_path . $module_name . DS . $module_name . '.zip';
        ...
        //6、复制文件成对应的版本包
        $file->handle_file($pack_zip, $this->app_pack_path . $module_name . DS . $info['version'] . '.zip');

        $datalog .= '<br>打包文件:' . $pack_zip;
