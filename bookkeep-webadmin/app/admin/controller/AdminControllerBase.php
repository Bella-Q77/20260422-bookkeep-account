...
class AdminControllerBase extends ControllerBase
{
    ...
    // 初始化目录
    final protected function initDirname()
    {
        $sessionDirname = RUNTIME_PATH . 'session' . DS;
        if (!file_exists($sessionDirname)) {
            mkdir($sessionDirname, 0777, true);
        }
        defined('SESSION_PATH') or define('SESSION_PATH', $sessionDirname);
    }
