 ...
 * @param bool $throwException 是否抛出异常（默认true）
 * @return object|false 服务实例或false
 * @throws Exception
 */
function get_service_class($module, $service, $throwException = true)
{
    $class = "app\\{$module}\\service\\{$service}";
    if (!class_exists($class)) {
        if ($throwException) {
            throw new \Exception("缺少{$module}模块支持，请先安装{$module}模块");
        }
        return false;
    }
    return new $class();
 ...
 * @return array 日期数组
 */
function get_date_range_days($startdate, $enddate)
{
    // 参数验证
    ...
    // 日期有效性检查
    if ($stimestamp === false || $etimestamp === false) {
        return [];
    }
...
function sign_field_filter($data = [])
{
    ...
    // 确保返回的是数组
    if (!is_array($data_sign_filter_field_array)) {
        $data_sign_filter_field_array = [];
    }
