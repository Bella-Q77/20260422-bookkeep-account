 ...
 * @param int $start_row 起始行
 * @param int $start_col 起始列
 * @param int $end_row 结束行（可选）
 * @param int $limit 限制条数（可选）
 */
function get_excel_data($file_url = '', $start_row = 1, $start_col = 0, $end_row = 0, $limit = 0)
{
    vendor('phpoffice/phpexcel/Classes/PHPExcel');
    ...
    // 确定结束行
    if ($end_row > 0) {
        $actual_end_row = min($end_row, $highestRow);
    } else {
        $actual_end_row = $highestRow;
    }

    // 如果设置了limit，则根据起始行和limit计算结束行
    if ($limit > 0) {
        $actual_end_row = min($actual_end_row, $start_row + $limit - 1);
    }

    $excel_data = [];

    for ($row = $start_row; $row <= $actual_end_row; $row++) {
