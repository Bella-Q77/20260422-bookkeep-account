 ...
 * 商家管理=》逻辑层
 */
class BkShop extends PortalmemberBookkeepBase
{
    /**
     * 商家列表
     * @param array $where
     ...
     * @return
     */
    public function getBkShopList($where = [], $field = true, $order = '', $paginate = DB_LIST_ROWS)
    {
    ...
    public function setBkShopDefault($data = [])
    {
        if (empty($data['id'])) {
            throw_response_error('选择执行的参数');
        }
        $where['member_id'] = ['=', MEMBER_ID];
        $result = $this->modelBkShop->setFieldValue($where, 'is_default', 0);

        //设置默认
        $where['id'] = ['=', $data['id']];
        $result && $this->modelBkShop->setFieldValue(['id' => $data['id']], 'is_default', 1);
        $result && action_log('设置默认商家', '设置默认商家，id：' . $data['id']);
        $url = url('show');
