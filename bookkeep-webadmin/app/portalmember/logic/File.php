 ...
 * 文件处理逻辑
 */
class File extends PortalmemberBase
{
    ...
    public function pictureUpload($name = 'file', $thumb_config = ['small' => 100, 'medium' => 500, 'big' => 1000])
    {
        ...
        $picture_info = $this->modelMemberPicture->getInfo(['sha1' => $sha1], 'id,name,path,sha1,member_id');
        ...
        $data = ['name' => $filename, 'path' => $picture_dir_name. SYS_DS_PROS . $filename, 'sha1' => $sha1, 'member_id' => MEMBER_ID];
     ...
     * small,medium,big
     */
    public function pictureUploadWater($name = 'file', $thumb_config = ['small' => 100, 'medium' => 500, 'big' => 1000])
    {
