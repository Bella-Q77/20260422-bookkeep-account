...
if (!function_exists('getYearStartEndTime')) {
    /**
     * 指定年月，年的起止时间
     * 支持格式：年(2023)、年月(2023-01)、年月日(2023-01-01)
     * @param string $date 日期字符串，支持年、年月、年月日格式
     * @return array 包含开始和结束时间的数组
     * Author: 开发人生 goodkfrs@qq.com
     * Date: 2021/8/9 0009 10:13
     */
    function getYearStartEndTime($date = '')
    {
        $year = date("Y"); // 默认使用当前年份

        if (!empty($date)) {
            // 首先检查是否为纯年份格式 (YYYY)
            if (preg_match('/^\d{4}$/', $date) && intval($date) > 1900 && intval($date) < 3000) {
                $year = intval($date);
            } else {
                // 尝试解析日期
                ...
                if ($timestamp !== false) {
                    ...
                    // 如果strtotime解析失败，尝试手动解析
                    // 支持 "YYYY-MM" 格式
                    if (preg_match('/^\d{4}-\d{1,2}$/', $date)) {
                        $parts = explode('-', $date);
                        if (count($parts) == 2 && intval($parts[0]) > 0) {
                            $year = intval($parts[0]);
                        }
                    }
                    // 支持 "YYYY-MM-DD" 格式
 ...
 * 2. 年（如：2023）
 * 3. 年-月（如：2023-05）
 * 4. 年-月-日（如：2023-05-15）
 * 5. 年/月/日（如：2023/05/15）
 * 6. 日期范围（如：2023/05/15-2023/08/15，取第一个日期作为标准）
 * @param string $date
 * @return array
 * Author: 开发人生 goodkfrs@qq.com
...
if (!function_exists('getQuarterStartEndTime')) {
    function getQuarterStartEndTime($date = '')
    {
        ...
        if (!empty($date)) {
            // 处理日期范围格式 (如: 2023/05/15-2023/08/15)
            if (strpos($date, '-') !== false && preg_match('/^(\d{4}[\/\-]\d{1,2}[\/\-]\d{1,2})\s*[-]\s*(\d{4}[\/\-]\d{1,2}[\/\-]\d{1,2})$/', $date, $matches)) {
                $date = $matches[1]; // 使用范围中的第一个日期
            }

            // 处理年份格式 (YYYY)
            if (preg_match('/^\d{4}$/', $date)) {
                $times = strtotime($date . '-01-01');
            }
            // 处理年-月格式 (YYYY-MM 或 YYYY/MM)
            elseif (preg_match('/^\d{4}[-\/]\d{1,2}$/', $date)) {
                $separator = strpos($date, '/') !== false ? '/' : '-';
                $times = strtotime(str_replace('/', '-', $date) . '-01');
            }
            // 处理年-月-日或年/月/日格式
            elseif (preg_match('/^\d{4}[-\/]\d{1,2}[-\/]\d{1,2}$/', $date)) {
                $times = strtotime(str_replace('/', '-', $date));
            }
            // 其他情况尝试直接解析
            else {
        ...
        // 季度开始时间
        $rangeTime['begin'] = date('Y-m-d H:i:s', mktime(0, 0, 0, $season * 3 - 2, 1, $year));
        // 季度结束时间
        $rangeTime['end'] = date('Y-m-d H:i:s', mktime(23, 59, 59, $season * 3, date('t', mktime(0, 0, 0, $season * 3, 1, $year)), $year));
...
/**
 * 指定日期所在周的开始时间和结束时间
 * @param string $date 指定日期，为空则使用当前日期
 * @return array 包含开始和结束时间的数组
 * Author: 开发人生 goodkfrs@qq.com
 * Date: 2021/8/9 0009 10:14
 */
if (!function_exists('getWeekStartEndTime')) {
    function getWeekStartEndTime($date = '')
    {
        if (empty($date)) {
            $times = time();
        } else {
        ...
        // 获取本周开始时间（周一）
        $beginWeek = mktime(0, 0, 0, date('m', $times), date('d', $times) - date('w', $times) + 1, date('Y', $times));
        // 获取本周结束时间（周日）
        $endWeek = mktime(23, 59, 59, date('m', $times), date('d', $times) - date('w', $times) + 7, date('Y', $times));

        $rangeTime['begin'] = date('Y-m-d H:i:s', $beginWeek);
        $rangeTime['end'] = date('Y-m-d H:i:s', $endWeek);
        return $rangeTime;
    }
}
...
/**
 * 获取两个日期之间所有日期
 * @param string|int $startDate 开始时间(支持时间戳或日期字符串)
 * @param string|int $endDate 结束时间(支持时间戳或日期字符串)
 * @return array 日期数组
 */
if (!function_exists('getDatesBetweenTwoDays')) {
    function getDatesBetweenTwoDays($startDate, $endDate)
    {
        ...
        // 如果开始时间大于结束时间，直接返回空数组
        if ($startTimestamp > $endTimestamp) {
            return $dates;
        }

        // 如果开始和结束是同一天
        $startYmd = date('Y-m-d', $startTimestamp);
        $endYmd = date('Y-m-d', $endTimestamp);

        if ($startYmd == $endYmd) {
            array_push($dates, $startYmd);
            return $dates;
        }
...
/**
 * 时间段转为两个数组
 * @param string $rangedate 支持多种格式：
 *   - 2025-10-12 - 2025-10-30
 *   - 2025/10/10 - 2025/10/30
 *   - 2025/10/10
 *   - 2025-10-12
 *   - 2025-10-12 - 2025-10-12
 *   - 2025/10/28-2025/10/29
 * @param string $pirx 两个时间段分隔符号，默认为 '-'
 * @param string $type 返回类型：str(返回字符串) | int(返回时间戳)
 * @return array 包含开始和结束时间的数组
 */
if (!function_exists('rangedate2arr')) {
    ...
    {
        // 标准化分隔符，将所有 / 替换为 -
        $normalized = str_replace('/', '-', trim($rangedate));

        // 处理带空格的格式 "2025-10-12 - 2025-10-30"
        if (strpos($normalized, ' - ') !== false) {
            $date_arr = explode(' - ', $normalized);
        }
        // 处理不带空格但有多个连字符的情况
        else if (substr_count($normalized, '-') > 2) {
            // 查找第三个连字符的位置作为分割点
            $pos = 0;
            for ($i = 0; $i < 3; $i++) {
                $pos = strpos($normalized, '-', $pos);
                if ($pos === false) break;
                if ($i < 2) $pos++; // 移动到下一个位置继续查找
            }

            if ($pos !== false) {
                $date_arr[0] = substr($normalized, 0, $pos);
                $date_arr[1] = substr($normalized, $pos + 1);
            } else {
                // fallback to simple split
                $date_arr = explode($pirx, $normalized, 2);
            }
        }
        // 只有两个连字符的情况，表示单个日期
        else if (substr_count($normalized, '-') == 2) {
        ...
        // 处理单个日期的情况
        if (empty($date_arr[1])) {
            // 如果只有一个日期元素，开始和结束时间都设为同一天
            $date_arr[0] = date('Y-m-d', strtotime($date_arr[0]));
            $date_arr[1] = $date_arr[0];
        } else {
            // 处理两个日期的情况
            $date_arr[0] = date('Y-m-d', strtotime($date_arr[0]));
            $date_arr[1] = date('Y-m-d', strtotime($date_arr[1]));
        }
        // 添加一天减一秒，使结束时间包含整天
        $date_arr[1] = date('Y-m-d H:i:s', strtotime(" +86399 second", strtotime($date_arr[1])));
 ...
 * @return string
 */
function covert_excel_date($dateValue)
{
    ...
    // 如果已经是标准日期格式，直接返回
    if (preg_match('/^\d{4}-\d{2}-\d{2}/', $dateValue) || preg_match('/^\d{4}\/\d{2}\/\d{2}/', $dateValue)) {
        // 检查是否为有效日期
        $date = date_create($dateValue);
        if ($date !== false && $date->format('Y-m-d') !== '0000-00-00') {
    ...
    if (is_numeric($dateValue)) {
        // Excel的基准日期是1900-1-1，序列号为1
        // 但由于历史原因，Excel将1900年视为闰年，所以需要减去2天修正
        // 需要确保转换后的日期是有效的
        if ($dateValue > 0) {
...
if (!function_exists('get_2arr_sum')) {
    ...
    {
        is_object($array) && $array = obj2arr($array);
